...
class FIJepaDataConfig:
    """Configure frozen-panel reconstruction, sampling, masking, and batching.

    The configuration intentionally combines these runtime concerns because
    they jointly define the shape and sampling semantics of every model batch.
    Validation rejects incompatible patch dimensions and impossible sampling
    bounds before an artifact is loaded.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.train_k_assets <= 0 or self.diagnostic_k_assets <= 0:
    ...
    def num_patches(self) -> int:
        """Return the fixed number of temporal patches in each sample window."""
        return self.lookback_days // self.patch_len

    @classmethod
    def from_yaml(cls, path: Path | str) -> FIJepaDataConfig:
        """Load a dataloader YAML file and normalize its artifact path."""
